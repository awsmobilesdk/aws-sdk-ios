...
/**
 ...
 @param request A container for the necessary parameters to execute the StartLiveTail service method.

 @return An instance of `AWSTask`. On successful execution, `task.result` will contain an instance of `AWSLogsStartLiveTailResponse`. On failed execution, `task.error` may contain an `NSError` with `AWSLogsErrorDomain` domain and the following error code: `AWSLogsErrorAccessDenied`, `AWSLogsErrorInvalidParameter`, `AWSLogsErrorResourceNotFound`, `AWSLogsErrorLimitExceeded`, `AWSLogsErrorInvalidOperation`.
 
 @see AWSLogsStartLiveTailRequest
 @see AWSLogsStartLiveTailResponse
 */
